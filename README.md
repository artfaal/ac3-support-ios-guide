...
Если ваша душа болить при упоминании *кодека AC3*, то вам сюда.

А если вы не поняли о чем идет речь, то возможно встречались с проблемой.

Всем, кто смотрит видео на iOS (про Android в самом конце), не в оригинальном формате и не через внутренний плейер это должно быть знакомо.
...
Поэтому в AppStore постепенно вырезали поддержку из всех плейеров.

### РЕШЕНИЕ.

1) Первый путь. Скачать прогрмма под названием AC3 Killer. Вот репозиторий на [GitHub](https://github.com/ac3killer/AC3-Killer), откуда его можно загрузить.
...
2) Самый простой и ффективный. Вам нужно скачать приложение VidOn Player.
...
По умолчанию, приложение этот формат не поддерживает (по понятным причинам). Но на (форуме разраотчиков)[http://forum.vidon.me/index.php?/topic/1279-feature-requests-vidon-player-ios-app/] есть ссылка на чудесный аддон. Вы просто скидываете этот файл в хранилище программы через iTunes (туда же, куда и все видеофайлы). И поддержка появляется.

Плюс у этого плейера нормальная поддержка субтитров, размер которых реально меняется. (В VLC поддержка ужасная).
